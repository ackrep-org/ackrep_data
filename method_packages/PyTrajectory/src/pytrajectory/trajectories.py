...
class Trajectory(Logger):
    ...
    def init_splines(self, export=False):
            ...
            for chain in self._chains:
                ...
                for i, elem in enumerate(chain.elements):  ##:: chain.elements= ('x1', 'x2', 'u1') or ('x3','x4')
                    if elem in self.sys.states:
                        splines[upper]._boundary_values[i] = bv[
                            elem
                        ]  ##:: for (x3,x4): splines['x3']._b_v= {0: (1.2566370614359172, 0.6283185307179586), 1: (0.0, 0.0)}, 0 is for x3, 1 is for x4, there is only splines['x3'],without splines['x4'], because upper here is only 'x3'
                ...
                for i, elem in enumerate(chain.elements):
                    if elem in self.sys.inputs:
                        ...
                        if i == 2:
                            ...
                            x_fnc[elem] = splines[
                                upper
                            ].ddf  ##:: x_fnc={'x1': method Spline.f, x2': Spline.df, 'x3': Spline.f, 'x4': Spline.df}
        ...
        if not export:
            ...
            C = auxiliary.Container(indep_vars=indep_vars, splines=splines, x_fnc=x_fnc, u_fnc=u_fnc, dx_fnc=dx_fnc)
            return C
            ...
            for var in self.sys.states + self.sys.inputs:
                for ic in self._chains:
                    if var in ic:  ##:: ('x1','x2','u1') and ('x3','x4')
                        subs[var] = subs[
                            ic.upper
                        ]  ##:: elements in the same chain have the same coefficients (number, not symbol).
    ...
    def save(self):
        ...
        # splines
        save["splines"] = OrderedDict((var, spline.save()) for var, spline in self.splines.items())
